...
const ponderImage = new docker.Image(imageName, {
    imageName: latestTag,
    build: {
 ...
 */
export const ponderInstance = new KubernetesService("Ponder", {
    // Global config
        ...
        containers: [
            {
                ...
                env: [
                    { name: "PONDER_DATABASE_URL", value: getDbUrl("ponder") },
                    {
                        name: "INTERNAL_RPC_URL",
                        value: `erpc-service.${blockchainNamespace.metadata.name}.svc.cluster.local:8080`,
                    },
                    {
                        name: "EXTERNAL_RPC_URL",
                        value: `erpc.${baseDomainName}`,
                    },
                ],
    ...
    // Service config
    service: {
        ports: [{ port: 80, targetPort: 42069, protocol: "TCP", name: "http" }],
    },
